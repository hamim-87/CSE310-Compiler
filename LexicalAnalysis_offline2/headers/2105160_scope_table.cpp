...
//main functions
bool ScopeTable :: Insert(string symbolName, string symbolType, ostream& out) noexcept{
...
void ScopeTable :: print(ostream& out) {
    out <<"ScopeTable # " << this->scopeUid << endl;
    for (int i = 0; i < this->bucketNumber; i++)
    {
        ...
        while (current != nullptr)
        {
            out<<"< "<<current->getSymbolName()<<" : "<<current->getSymbolType()<<" >";
            current = current->getNextSymbolInfo();
        }
        out<<endl;
    }
